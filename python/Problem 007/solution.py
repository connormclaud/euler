...
sieve = [True, True, False] + [
    False if x % 2 else True for x in xrange(3, estimate)]
prime_count = 1
for candidate in xrange(3, sqrt_estimate, 2):
...
for candidate in xrange(sqrt_estimate, estimate):
    if not sieve[candidate]:
        prime_count += 1
        if prime_count == nth:
            result = candidate
